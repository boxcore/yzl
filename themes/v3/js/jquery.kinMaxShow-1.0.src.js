...
(function($){
	$.fn.kinMaxShow = function(user_options){
			...
			$leftBtn  = $("#main_king_shaw").find('a.prev');
			$rightBtn = $("#main_king_shaw").find('a.next');
			...
			k.setOpacity = function(){
				...
				$leftBtn.click(function () {
	                if ($(':animated').length === 0) {
						...
						//设置当前显示的z-index为1
						$('.'+k.prename+'image_item',k.self).eq(k.index).css({opacity:0,'z-index':0});
	                }
	            });
...
$(function(){
  ...
  $(window).resize(function(){
    var elem = $(this);
    var kinL = (elem.width()-980)/2;
    $("#main_king_shaw").css({"left":kinL});
    // Update the info div width and height - replace this with your own code
    // to do something useful!
    $('#hmsg').text( 'window width: ' + elem.width() + ', height: ' + elem.height() );
  });
