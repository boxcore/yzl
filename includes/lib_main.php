 ...
 * @return  void
 */
function show_sign_msg($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true, $sign_mark = false)
{
    assign_template();
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
	$GLOBALS['smarty']->assign('sign_mark', $sign_mark);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('sign_msg.dwt');
 ...
 * @return  array
 */
function article_categories_tree1($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('article_cat') . " WHERE cat_id = '$cat_id'";
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www.xxoopp.org/license.php?product=ecshop_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
